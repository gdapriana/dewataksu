...
import { BookmarkValidations } from "@/utils/validation/bookmark.validation";
import { validateRequest } from "@/utils/validation/validate";
import { headers } from "next/headers";
import { NextRequest, NextResponse } from "next/server";
import { ZodError } from "zod";

const table = prisma.bookmark;
const validation = BookmarkValidations;

export async function GET(
  req: NextRequest,
  { params }: { params: { schema: string; id: string } }
) {
  try {
...
export async function POST(
  ...
  try {
    ...
    if (checkItem) return ErrorResponseMessage.ALREADY_EXISTS("bookmark");
